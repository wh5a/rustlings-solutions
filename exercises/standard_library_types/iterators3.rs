...
mod tests {
    ...
    fn result_with_list() {
        ...
        let x: Result<Vec<i32>, DivisionError> = division_results.filter(|x| x.is_ok()).collect();
    ...
    fn list_of_results() {
        let numbers = vec![27, 297, 38502, 81];
        let division_results = numbers.into_iter().map(|n| divide(n, 27));
        let x = division_results.filter(|x| x.is_ok()).collect::<Vec<Result<i32, DivisionError>>>();
